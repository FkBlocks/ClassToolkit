...
> 🗄️ **本文档已归档**：悬浮球方案已被《左右对称侧边栏》取代（见 [REQUIREMENTS.md](REQUIREMENTS.md) 需求一），文中的形状系统、内嵌内容设计不再采用。
> 其中 **§5「音量控制集成（智能检测外部媒体）」** 的 Core Audio 会话检测、系统进程过滤、
> 状态机与 Slider 双向同步研究仍与音量集成需求直接相关，保留参考价值。
